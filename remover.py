...
def blur_mask(alpha, W):
	...
	video = "/users/yang/Downloads/white.mp4" #白色视频作为遮罩
	video = "/users/yang/Downloads/qianliexian.mp4"
	cap = cv.VideoCapture(video)
	...
	print(med.shape)
        ...
        med = np.repeat(med, 3, axis=-1).reshape(med.shape[0], med.shape[1], 3)

	cv.imwrite("/users/yang/Downloads/removewater.png",med)
...
def process_video(video, alpha, W, bg, out_video=None):
	...
	if out_video is not None:
		...
		vwriter = cv.VideoWriter("/users/yang/Downloads/222.mp4", video_FourCC,
...
def main():
	...
	dirname = "/users/yang/Downloads/"
	for root, dirs, names in os.walk(dirname):
		if dirs==[]:
			os.makedirs(root.replace("/users/yang/Downloads/qianliexian.mp4", "/users/yang/Downloads/222.mp4"), exist_ok=True)
			for name in names:
				video = root+"/"+name
				out_video = root.replace("/users/yang/Downloads/qianliexian.mp4", "/users/yang/Downloads/222.mp4")+"/"+name
				print(video)
